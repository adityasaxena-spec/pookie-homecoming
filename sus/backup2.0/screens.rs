...
impl Plugin for ScreensPlugin {
    fn build(&self, app: &mut App) {
        app.add_systems(Update, (check_game_over, check_win).run_if(in_state(GameState::Playing)))
           ...
           // Global toggle for pausing and unpausing
           ...
           .add_systems(Update, exit_on_space.run_if(in_state(GameState::Win)))
...
fn check_game_over(stability: Res<ShipStability>, mut next_state: ResMut<NextState<GameState>>) {
    if stability.current <= 0.0 {
        next_state.set(GameState::GameOver);
    }
}

fn check_win(wave: Res<WaveManager>, mut next_state: ResMut<NextState<GameState>>) {
    if wave.current_wave > wave.max_waves {
...
// FIX: Updated to MessageWriter and .write() for Bevy 0.17+
...
// Listens for the Escape key to toggle pause state
fn toggle_pause(
    ...
    if keys.just_pressed(KeyCode::Escape) {
        match state.get() {
            ...
            _ => {} // Do nothing if in intro, game over, or win screens
        }
    }
}
...
// Spawns the pause screen with a slightly transparent background
fn setup_pause(mut commands: Commands) {
    spawn_text_screen(
        ...
        "Press ESC to Resume",
        Color::srgba(0.0, 0.0, 0.0, 0.85), // Semi-transparent black so you can still see the game!
    );
}
...
fn setup_win(mut commands: Commands) {
    spawn_text_screen(
        &mut commands,
        "YOU SURVIVED",
        "Hostile forces cleared.\n\nYou have successfully reached Pookietopia!\nWelcome home, Commander.",
        "Press SPACE to Exit",
