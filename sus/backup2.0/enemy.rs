...
impl Plugin for EnemyPlugin {
    fn build(&self, app: &mut App) {
        app.insert_resource(WaveManager {
            ...
            is_active: false, // Start inactive until player hits a checkpoint
        })
        .add_systems(
            Update,
            (
...
fn spawn_enemies(
    ...
    // If we've beaten the game, or the player hasn't hit the checkpoint yet, do not spawn.
    if !wave.is_active || wave.current_wave > wave.max_waves {
        return;
    }

    if wave.enemies_to_spawn == 0 && wave.enemies_alive == 0 {
        // Wave Defeated! Turn off active status so the compass reappears aiming at the next checkpoint
        wave.is_active = false;
        wave.current_wave += 1;
        if wave.current_wave <= wave.max_waves {
            // Next wave: increase spawn count
