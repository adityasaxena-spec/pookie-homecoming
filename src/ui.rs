    ...
    fn build(&self, app: &mut App) {
        ...
        .add_systems(Startup, setup_ui)
            .add_systems(Update, (
                    ...
                    update_status_warning,
            ).run_if(in_state(GameState::Playing)))
            // Reset systems attached to exiting the end screens
            .add_systems(OnExit(GameState::GameOver), reset_ui)
            .add_systems(OnExit(GameState::Score), reset_ui);
    }
}
...
#[derive(Component)]
struct StatusWarningText;

fn reset_ui(
    mut commands: Commands,
    mut checkpoints: ResMut<Checkpoints>,
    popup_query: Query<Entity, With<CheckpointPopup>>
) {
    checkpoints.locations = vec![
        Vec2::new(1500.0, 1500.0),
        Vec2::new(3000.0, -1000.0),
        Vec2::new(-2000.0, 2000.0),
    ];
...
fn update_status_warning(
    stability: Res<ShipStability>,
    mut query: Query<(&mut Text, &mut TextColor), With<StatusWarningText>>,
) {
    for (mut text, mut color) in &mut query {
        if stability.is_red() {
            text.0 = "⚠ CRITICAL: CONTROLS INVERTED".to_string();
            color.0 = Color::srgb(1.0, 0.3, 0.3);
        } else if stability.is_orange() {
            text.0 = "⚠ WARNING: WEAPON AIM UNSTABLE".to_string();
            color.0 = Color::srgb(1.0, 0.6, 0.1);
        } else if stability.is_yellow() {
            text.0 = "⚠ CAUTION: WEAPON MISFIRING".to_string();
